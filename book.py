...
class BookingListener:
    ...
    def confirm(self, timeout=2):
        ...
        expected_options = ["There are empty spots", "maximum", "too far ahead", "back-to-back"]
        ...
        try:
            ...
            for expected_text in expected_options:
                if expected_text.lower() in toast.text.lower():
                    ...
                    raise ToastError(f"Booking failed: {toast.text}", toast_text=toast.text)
...
def find_slots(booking, driver):
    ...
    print(f"Booking time: {booking_time.strftime('%H:%M %p')} on {booking.date}")
    try:
        columns = WebDriverWait(driver, 10).until(
            EC.presence_of_all_elements_located(
                (By.CSS_SELECTOR, ".column.slots")
                )
            )
        columns.pop()
        elements = []
        ...
        for element in elements:
            ...
            if start_time.strftime('%H:%M %p') == booking_time.strftime('%H:%M %p'):
                return element
        return None
    except Exception as e:
        print(f"An error occurred: {str(e)}")
...
def reserve_slot(driver, element):
    ...
    try:
        ...
        button = wait.until(lambda driver: modal.find_element(By.CSS_SELECTOR, "button.mat-raised-button"))
        button.click()
        try:
...
def check_prime_time(surface, driver):
    ...
    try:
        prime_time = wait.until(lambda driver: surface.find_element(By.CSS_SELECTOR, ".usq-prime-time"))
        return prime_time
    except:
        return False
...
def book_prime_time(modal, driver):
    ...
    if check_prime_time(modal, driver):
        try:
            player_input = wait.until(lambda driver: modal.find_element(By.CSS_SELECTOR, 'input.mat-input-element:not([readonly="true"])'))
            player_input.clear()
            player_input.send_keys(partner_name)
            x_path = f'//span[contains(text(), "{partner_name}")]'
            dropdown = wait.until(lambda driver: player_input.find_element(By.XPATH, x_path))
            if dropdown:
                dropdown.click()
            return True
